...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck=True):
    ...
    bn1 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=eps, name=name + '_bn1', use_global_stats=use_global_stats)
    act1 = mx.sym.Activation(data=bn1, act_type='relu', name=name + '_relu1')
    conv2 = mx.sym.Convolution(data=act1, num_filter=int(num_filter*0.25), kernel=(3,3), stride=(1,1), pad=(1,1),
                                no_bias=True, workspace=workspace, name=name + '_conv2')
    bn2 = mx.sym.BatchNorm(data=conv2, fix_gamma=False, eps=eps, name=name + '_bn2', use_global_stats=use_global_stats)
    act2 = mx.sym.Activation(data=bn2, act_type='relu', name=name + '_relu2')
    conv3 = mx.sym.Convolution(data=act2, num_filter=num_filter, kernel=(1,1), stride=(1,1), pad=(0,0), no_bias=True,
                                workspace=workspace, name=name + '_conv3')
    bn3 = mx.sym.BatchNorm(data=conv3, fix_gamma=False, eps=eps, name=name + '_bn3', use_global_stats=use_global_stats)
    ...
    if dim_match:
        ...
        shortcut = mx.sym.BatchNorm(data=conv1sc, fix_gamma=False, eps=eps, name=name + '_downsample_1', use_global_stats=use_global_stats)
