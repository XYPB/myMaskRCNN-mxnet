...
def get_resnet_train(anchor_scales, anchor_ratios, rpn_feature_stride,
    ...
    for stride in RPN_FEAT_STRIDE:
        ...
        rpn_cls_prob_list.append(rpn_cls_prob_reshape)
        ...
        # rpn proposal
        rois = mx.symbol.contrib.MultiProposal(cls_prob=rpn_cls_prob_reshape, bbox_pred=rpn_bbox_pred, im_info=im_info, name='rois',
                                                feature_stride=stride, scales=anchor_scales[i], ratios=anchor_ratios,
    ...
    # rpn_bbox_loss = mx.symbol.Concat(*rpn_bbox_loss_list, dim=0)
    # rpn_cls_prob = mx.symbol.Concat(*rpn_cls_prob_list, dim=0)
    ...
    group = mx.symbol.Group(
                            [rpn_cls_prob_list[0], rpn_cls_prob_list[1], rpn_cls_prob_list[2], rpn_cls_prob_list[3], rpn_cls_prob_list[4],
